...
int ghct_temp_status = HCT_TEMP_STATE_UNKNOWN;
...
#if 0
	/* JEITA battery temp Standard */
	if (BMT_status.temperature > MAX_CHARGE_TEMPERATURE) {  // above 55
		battery_log(BAT_LOG_CRTI,
				"[hctBATTERY] Battery Over high Temperature(%d) !!\n\r",
				TEMP_POS_60_THRESHOLD);
		ghct_temp_status = HCT_TEMP_ABOVE_POS_55;
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
		return ;
	}
	else if (BMT_status.temperature >= (MAX_CHARGE_TEMPERATURE - TEMP_DALTA_THRES_DEGREE)) {
			  if(ghct_temp_status == HCT_TEMP_ABOVE_POS_55){
			g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature from 55 , must blow 53 can rechage,cur=%d,not allow charging yet!!\n\r",
					BMT_status.temperature);
			return ;
		}
		 else {
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature between %d and %d !!\n\r",
					TEMP_POS_50_THRESHOLD, MAX_CHARGE_TEMPERATURE);
			ghct_temp_status = HCT_TEMP_POS_50_TO_POS_55;
			g_temp_CC_value = AC_TEM_PROTECT_CHARGER_CURRENT;
		}
	} else if (BMT_status.temperature >= TEMP_POS_50_THRESHOLD) {
	
		if (ghct_temp_status == HCT_TEMP_POS_0_TO_POS_50) {
			ghct_temp_status = HCT_TEMP_POS_50_TO_POS_55;
			g_temp_CC_value = AC_TEM_PROTECT_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature rasing above 50, need down!!\n\r");
		}
		else if((ghct_temp_status == HCT_TEMP_ABOVE_POS_55 )&&(BMT_status.temperature <= (MAX_CHARGE_TEMPERATURE - TEMP_DALTA_THRES_DEGREE)))
		{
			ghct_temp_status = HCT_TEMP_POS_50_TO_POS_55;
			g_temp_CC_value = AC_TEM_PROTECT_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature failing down 53 , return to protect current!!\n\r");
		
		}		
	} else if(BMT_status.temperature >= TEMP_POS_0_THRESHOLD){
		  
		if ((ghct_temp_status == HCT_TEMP_POS_50_TO_POS_55)&&(BMT_status.temperature <= (TEMP_POS_50_THRESHOLD - TEMP_DALTA_THRES_DEGREE))) {
			ghct_temp_status = HCT_TEMP_POS_0_TO_POS_50;
			g_temp_CC_value = AC_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature failng below 47 , return to nomal!!\n\r");
		}
		else if((ghct_temp_status == HCT_TEMP_NEG_10_TO_POS_0 )&&(BMT_status.temperature <= (TEMP_POS_0_THRESHOLD + TEMP_DALTA_THRES_DEGREE)))
		{
			ghct_temp_status = HCT_TEMP_POS_0_TO_POS_50;
			g_temp_CC_value = AC_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature rasing above 2 , return to nomal!!\n\r");
		
		}
	} else if(BMT_status.temperature >= HCT_TEMP_NEG_10_THRESHOLD){
		if ((ghct_temp_status == HCT_TEMP_POS_0_TO_POS_50)&&(BMT_status.temperature <= TEMP_POS_0_THRESHOLD )) {
			ghct_temp_status = HCT_TEMP_NEG_10_TO_POS_0;
			g_temp_CC_value = AC_LOW_TEM_PROTECT_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature failng below 0 , use  800MA charging!!\n\r");
		}
		else
		{
			ghct_temp_status = HCT_TEMP_NEG_10_TO_POS_0;
			g_temp_CC_value = AC_LOW_TEM_PROTECT_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
					"[hctBATTERY] Battery Temperature is above -10 , use  800MA charging!!\n\r");
		
		}

	} else
	{  // means temprature between -10 to -20
		ghct_temp_status = HCT_TEMP_BELOW_NEG_10;
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
		battery_log(BAT_LOG_CRTI,
				"[hctBATTERY] Battery Temperature is below -10 to -20 , return to nomal!!\n\r");
		
	}

	
	battery_log(BAT_LOG_CRTI,
			"[hctBATTERY] Battery Temperature(%d) , tem_st=%d, cc_val=%d !!\n\r",
			BMT_status.temperature,ghct_temp_status, g_temp_CC_value );
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			battery_log(BAT_LOG_CRTI,"[BATTERY] select_charging_curret_bcct not used, but use hct mothod !\n");
